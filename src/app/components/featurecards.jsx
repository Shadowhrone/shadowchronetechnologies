...
const Featurecards = () => {
  ...
  transition={{
    ...
    ease: "easeOut",
      ...
      >
        <Image src={card.img} width={60} height={60} alt={card.title} className="xl:w-15 xl:h-15 w-7.5 h-7" />
        <h1 className="md:text-3xl text-xl text-center text-[#030D3C] font-bold">{card.title}</h1>
        <h2 className="text-[#48454B] text-xs md:text-base opacity-70 text-center font-medium">
          {card.description}
        </h2>
      </motion.div>
    ))}
